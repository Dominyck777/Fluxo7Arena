...
export const IsisResponseButtons = ({ buttons, onSelect, disabled = false, hideText = false, hidden = false }) => {
  return (
    <motion.div
      layout
      initial={{ opacity: 0, y: 10, clipPath: 'inset(0% 0% 0% 0%)' }}
      animate={{
        opacity: hidden ? 0 : 1,
        y: hidden ? -8 : 0,
      ...
      className={`flex flex-wrap gap-2 md:gap-2.5 mb-4 md:mb-6 ml-12 md:ml-[68px] transition-all duration-500 ${hidden ? 'pointer-events-none' : ''}`}
    >
